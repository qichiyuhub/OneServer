...
#
# 索引里的 100755 / 100644 是提交进仓库的事实，工作区的权限位随检出环境变：
# `core.filemode=false` 时 chmod 根本不进索引，跨文件系统挂载与 umask 也能
# 让两者对不上。`tests/lint.sh` 的「可执行位」检查同样认索引，两处必须一致。
...
# 服务器脚本写死 PATH 是为了防 PATH 劫持提权；这个工具只在开发机上跑、
# 不以 root 跑、不落到用户机器上，那条威胁不成立。反过来写死的代价是真的：
# git 装在非标准前缀的开发机上它会当场报「不在 git 仓库里」，而真因是
# 找不到 git。
